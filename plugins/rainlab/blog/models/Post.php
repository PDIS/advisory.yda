...
use RainLab\User\Models\User as ClientUser;
use Carbon\Carbon;
...
class Post extends Model
{
    ...
    {
        return $this->belongsToMany('bedard_blogtags_post_tag', 'post_id');
    }
    ...
    // post to post function to plugin
    public function getPostChildOptions() {
        return Db::table('rainlab_blog_categories')
                ->join('rainlab_blog_posts_categories', 'rainlab_blog_categories.id', '=', 'category_id')
                ->join('rainlab_blog_posts', 'rainlab_blog_posts_categories.post_id', '=', 'rainlab_blog_posts.id')
                ->select('rainlab_blog_posts.id', 'title')->where('rainlab_blog_categories.id', '=', 1)->lists('title', 'rainlab_blog_posts.id');
    }

    public function getChildPosts() {
        $childIdArray = array();
        $children = Db::table('rainlab_blog_post_children')
            ->join('rainlab_blog_posts', 'id', '=', 'post_child_id')
            ->where('post_id', '=', $this->id)->orderBy('rainlab_blog_posts.published_at', 'desc')->get();
        foreach($children as $child) {
            $childIdArray[] = $child->post_child_id;
        }
        
        return Post::find($childIdArray);
    }

    public function getUsers() {
        $userIdArray = array();
        $users = Db::table('rainlab_blog_posts')
            ->join('rainlab_blog_user_post', 'rainlab_blog_posts.id', '=', 'post_id')
            ->join('users', 'rainlab_blog_user_post.user_id', '=', 'users.id')
            ->where('rainlab_blog_posts.id', '=', $this->id)->get();
        foreach($users as $user) {
            $userIdArray[] = $user->id;
        }
        
        return ClientUser::find($userIdArray);
    }

    
    public function getReconsiderationUsers() {
        $userIdArray = array();
        $users = Db::table('rainlab_blog_posts')
            ->join('rainlab_blog_reconsideration_user_post', 'rainlab_blog_posts.id', '=', 'post_id')
            ->join('users', 'rainlab_blog_reconsideration_user_post.user_id', '=', 'users.id')
            ->where('rainlab_blog_posts.id', '=', $this->id)->get();
        foreach($users as $user) {
            $userIdArray[] = $user->id;
        }
    ...
    public function getTrackUsers() {
        $userIdArray = array();
        $users = array();
        $childPosts = $this->getChildPosts();

        foreach($childPosts as $childPost) {
            foreach($childPost->getUsers() as $user) {
