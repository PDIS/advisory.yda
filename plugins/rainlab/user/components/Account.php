...
class Account extends ComponentBase
{
    ...
    public function onUpdate()
    {
        ...
        $tagNameString = Input::get('user_tag');
        $tagNames = explode(",",$tagNameString);
		$tagNameArray = array();
        Db::table('bedard_blogtags_user_tag')->where('user_id', $this->user()->id)->delete();
		foreach($tagNames as $tagName) {
			$tag = Tag::where("name", "=", $tagName)->first();
			if ($tag != null) {
				array_push($tagNameArray, ['tag_id' => $tag->id, 'user_id' => $this->user()->id]);
			}
        }
		DB::table('bedard_blogtags_user_tag')->insert($tagNameArray);
        $user->fill(post());
        $user->save();
